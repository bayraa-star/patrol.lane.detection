...
# Expose the installed OpenCV Python extension if it lives under cv2/python-X.Y.
RUN python3 - <<'PY'
import sys
...
# Copy requirements before install so dependency failures are not hidden.
COPY requirements.txt /tmp/requirements.txt

# Python deps without keeping pip OpenCV wheels over the system build
